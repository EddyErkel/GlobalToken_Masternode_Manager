...
DONADDR1="GLT: Gggy14eGm6ykqKrAVRzDHuB3hgHod1sBzE"
...
NODE_NAME="GlobalToken"
COIN_NAME="globaltoken"
COIN_TICKER="GLT"
COIN_PATH="/usr/local/bin"
COIN_DAEMON="globaltokend"
COIN_CLI="globaltoken-cli"
COIN_PID="globaltokend.pid"
COIN_FOLDER="/root/.globaltoken"
COIN_CONFIG="globaltoken.conf"
COIN_SERVICE="globaltoken.service"
COIN_PORT="9319"
RPC_PORT="9320"
...
COIN_URL16="https://github.com/globaltoken/globaltoken/releases/download/3.2.1/globaltoken-3.2.1-x86_64-linux-gnu.tar.gz"  	# Binaries compressed file for Ubuntu 16.
COIN_URL18="https://github.com/globaltoken/globaltoken/releases/download/3.2.1/globaltoken-3.2.1-x86_64-linux-gnu.tar.gz"  	# Binaries compressed file for Ubuntu 18.
COIN_ZIPDIR="bin/"                                                                                     					# Path inside the zipfile that contains the binaries.


# Masternode Bootstrap URL
#CHAIN_URL="https://globaltoken.org/downloads/bootstrap.dat"														# Bootstrap URL. The Bootstrap is compressed file containing chain directory's.
...
MAIN_WWW="https://globaltoken.org/"                                                             					# Main website URL
REPO_WWW="https://github.com/globaltoken/globaltoken"                                          						# Repository URL
EXPL_WWW="https://explorer.globaltoken.org/"                                                        				# Explorer URL
MNST_WWW="https://mnrank.com/coin/GLT/"                                               								# Masternodes Statitics URL
CMKT_WWW="https://coinmarketcap.com/currencies/globaltoken/"                                              			# Coin Market URL
...
MNMGTCOMMANDS="help autocompletion install summary update systemupdate addnodes bootstrap createbootstrap stop start status monitor showconf replace createswap optimize checkblockhash disclaimer donation dupmn 'status verbose"
MNCLICOMMANDS="help getblockchaininfo getblockcount getinfo getmasternodecount getnetworkinfo getconnectioncount getpeerinfo 'masternode genkey' 'masternode status' 'mnsync status'"
...
function display_logo() {
    ...
    echo -e "${W} ╚═════╝ ╚══════╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝    ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝"; sleep $DELAY
    echo -e "${W}                                                                               		    "; sleep $DELAY
    echo -e "${W}                  GlobalToken (GLT) is a multi-algorithm cryptocurrency.                   "; sleep $DELAY
    echo -e "${W}            It is the world's largest proof-of-work cryptocurrency with support            "; sleep $DELAY
    echo -e "${W}            for 60 mining algorithms, the most algorithms of any cryptocurrency.           "; sleep $DELAY
    echo -e "${W}                                                                                           "; sleep $DELAY
...
function display_help() {
	...
	echo -e "${C}systemupdate           ${D}: Update system packages${N}"
    echo -e "${C}replace [strA] [strB]  ${D}: Replace 'string A' with 'string B' in $COIN_CONFIG${N}"	
    if [[ $CHBLK_ENABLE == "true" ]]; then
		echo -e "${C}checkblockhash         ${D}: Check blockhash against explorer${N}"
	fi
    if [[ $DUPMN_ENABLE == "true" ]]; then
    ...
    if [ ! -z "$MAIN_WWW"  ]; then echo -e "${D}- Main website              : ${Y}$MAIN_WWW${N}" ; fi
...
function install_tools() {
    ...
    echo
    echo
    echo -e "${G}INSTALL SYSTEM TOOLS${N}"
    echo
    echo -e "${D}This will install zip, unzip, wget, nano, htop, jq, curl, rsync and software-properties-common.${N}"
    echo 
    echo -e "${D}Do you want install system tools (recommended)? [Y/n]${N}"
    read -s -n1 SELECTION

    if [[ $SELECTION == @("Y"|"y"|"") ]]; then
        echo
        echo -e "${D}Installing system tools...${N}"
        apt-get -y install zip unzip 
        apt-get -y install wget nano htop jq curl rsync
        apt-get -y install software-properties-common
        echo
        echo -e "${Y}Finished installing system tools.${N}"
    else
        echo
        echo -e "${Y}System tools installation skipped.${N}"
    fi
}
...
function verifyblockhash() {
	CHECK_CLI=$1
	eval EXPL_BLCKCOUNT="$2"
	eval EXPL_BLCKHASH="$3"
	BLCKCOUNT=$4

	if [[ $BLCKCOUNT ]] 
	then
		COIN_BLCKHASH=$($CHECK_CLI getblockhash $BLCKCOUNT 2>/dev/null )  
		if [[ "$EXPL_BLCKHASH" == "$COIN_BLCKHASH" ]]
		then
			echo -e "${D}$CHECK_CLI $BLCKCOUNT $EXPL_BLCKHASH $COIN_BLCKHASH${N}"
		else
			if [ -z "$COIN_BLCKHASH" ]
			then
				COIN_BLCKHASH="Block height out of range"
			fi
			echo -e "${D}$CHECK_CLI $BLCKCOUNT $EXPL_BLCKHASH ${R}$COIN_BLCKHASH${N}"
		fi
	else
		COIN_BLCKCOUNT=$($CHECK_CLI getblockcount)
		if [ "$EXPL_BLCKCOUNT" -le "$COIN_BLCKCOUNT" ]
		then
			BLCKCOUNT=$EXPL_BLCKCOUNT
			COIN_BLCKHASH=$($CHECK_CLI getblockhash $BLCKCOUNT)
		
			if [[ "$EXPL_BLCKHASH" == "$COIN_BLCKHASH" ]]
			then
				echo -e "$CHECK_CLI ${D}$BLCKCOUNT $EXPL_BLCKHASH ${D}$COIN_BLCKHASH${N}"
			else
				echo -e "$CHECK_CLI ${D}$BLCKCOUNT $EXPL_BLCKHASH ${R}$COIN_BLCKHASH${N}"
			fi

		else
			BLCKCOUNT=$COIN_BLCKCOUNT
			EXPL_BLCKHASH=$(wget -qO- $EXPL_WWW/api/getblockhash?index=$BLCKCOUNT 2>/dev/null)
			COIN_BLCKHASH=$($CHECK_CLI getblockhash $BLCKCOUNT)
		
			if [[ "$EXPL_BLCKHASH" == "$COIN_BLCKHASH" ]]
			then
				echo -e "$CHECK_CLI ${Y}$BLCKCOUNT ${D}$EXPL_BLCKHASH ${Y}$COIN_BLCKHASH${N}"
			else
				echo -e "$CHECK_CLI ${Y}$BLCKCOUNT ${D}$EXPL_BLCKHASH ${R}$COIN_BLCKHASH${N}"
			fi
		fi	
	fi
	unset CHECK_CLI
	unset EXPL_BLCKCOUNT
	unset EXPL_BLCKHASH
	unset BLCKCOUNT
}
...
function checkblockhash() {
    ...
    echo
    echo  
    echo -e "${G}CHECK BLOCKHASH${N}"
    echo
    if [[ $CHBLK_ENABLE == "true" ]]; then
		BLOCK=$ARG2
		EXPL_WWW=${EXPL_WWW%/}
		EXPL_BLOCKCOUNT=$(wget -qO- $EXPL_WWW/api/getblockcount)
		EXPL_BLOCKHASH=$(wget -qO- $EXPL_WWW/api/getblockhash?index=$EXPL_BLOCKCOUNT)
		
		echo
		echo -e "${D}Explorer blockcount: $EXPL_BLOCKCOUNT${N}"
		echo -e "${D}Explorer blockhash : $EXPL_BLOCKHASH${N}"
		echo
		echo
		
		if [[ $BLOCK ]] 
		then
			echo -e "${D}Verifying against block ${Y}$BLOCK.${N}"
			EXPL_BLOCKCOUNT=$BLOCK
			EXPL_BLOCKHASH=$(wget -qO- $EXPL_WWW/api/getblockhash?index=$EXPL_BLOCKCOUNT)
			if [ "$EXPL_BLOCKHASH" == "There was an error. Check your console." ]
			then
				EXPL_BLOCKHASH="Block height out of range                                       "
			fi
		else
			echo -e "${D}Verifying against highest available ${Y}common ${D}block.${N}"
		fi
		echo
		echo -e "${D}-----------------------------------------------------------------------------------------------------------------------------------------------------------------${N}"	
		echo -e "${D}Masternode              Block   Explorer blockhash                                               Coin blockhash${N}"
		echo -e "${D}-----------------------------------------------------------------------------------------------------------------------------------------------------------------${N}"

		if [ -f "/usr/bin/${COIN_CLI}-all" ]
		then
			MULTI_CLI=$(ls --file /usr/bin/${COIN_CLI}-* | grep -v 'cli-all')
			MULTI_CLI=($(echo ${MULTI_CLI[@]} | tr " " "\n" | sort -V))  
			for CLI in "${MULTI_CLI[@]}"
			do
				verifyblockhash $CLI "\${EXPL_BLOCKCOUNT}" "\${EXPL_BLOCKHASH}" $BLOCK
			done
		else

				verifyblockhash $COIN_CLI "\${EXPL_BLOCKCOUNT}" "\${EXPL_BLOCKHASH}" $BLOCK
		fi
		echo -e "${D}-----------------------------------------------------------------------------------------------------------------------------------------------------------------${N}"
	else
        echo -e "${Y}Sorry, check blockhash command is not available for $NODE_NAME.${N}"
	fi
}
...
if [[ $ARG1 == "checkblockhash"   ]]; then VALIDCMD="true"; checkblockhash              ; fi
if [[ $ARG1 == "systemupdate"     ]]; then VALIDCMD="true"; update_system             	; fi
